 ...
 */
public class PolygonRendererPoint2D extends AbstractShapeRenderer {
     ...
     */
    public PolygonRendererPoint2D(int polyType) {
        m_polyType = polyType;
    }
    ...
    protected Shape getRawShape(VisualItem item) {
    	Collection<Point2D> poly = (Collection<Point2D>) item.get(m_polyfield);
        if ( poly == null ) { return null; }
        
        float x = (float)item.getX();
        float y = (float)item.getY();
        
        Iterator<Point2D> points = poly.iterator();
        Point2D point = points.next();
        
        // initialize the path
        m_path.reset();
        m_path.moveTo(x+point.getX(),y+point.getY());
        ...
        if ( m_polyType == Constants.POLY_TYPE_LINE )
        {
            ...
            while (points.hasNext()) {
            	point = points.next();
                m_path.lineTo(x+point.getX(),y+point.getY());
            }
        }
