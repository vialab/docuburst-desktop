	/* CVS $Id: RotationLabelRenderer.java,v 1.6 2009/08/29 22:14:07 cmcollin Exp $ */
...
public class RotationLabelRenderer extends AbstractShapeRenderer {
    ...
    public static final Schema SCHEMA = new Schema(new String[] {ROTATION}, new Class[] {double.class});
    
    /**
     * Whether rotations should be restricted to -90 < theta < 90 or not (i.e. keep text upright).  True by default.
     */
    private boolean forceUpright = true;
        
    /** 
     * The rotation of the current item.
     */
    protected double itemRotation; // current item rotation in degrees
    ...
    public void setForceUpright(boolean forceUpright) {
    	this.forceUpright = forceUpright;
    }
    
    public boolean isForceUpright() {
    	return forceUpright;
    }
    ...
    public void render(Graphics2D g, VisualItem item) {
    	...
    	if (rotation != 0)
        	g.rotate(Math.toRadians(rotation), shape.getCenterX(), shape.getCenterY());
    ...
    protected AffineTransform getTransform(VisualItem item) {
    	...
    	if (forceUpright) {
    		if (((itemRotation > 90) && (itemRotation < 270)) || ((itemRotation < -90) && (itemRotation > -270)))
    			itemRotation += 180;
    	}
    		
    	itemTransform.rotate(Math.toRadians(itemRotation), itemRawShape.getCenterX(), itemRawShape.getCenterY());
