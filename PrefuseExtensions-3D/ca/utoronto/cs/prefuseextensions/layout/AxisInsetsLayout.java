/* CVS $Id: AxisInsetsLayout.java,v 1.1 2006/12/04 06:49:17 cmcollin Exp $ */
...
public class AxisInsetsLayout extends Layout {
     ...
     */
    public AxisInsetsLayout(String group, String field) {
        super(group);
        m_field = field;
        insets = new AxisInsets(0,0,0,0);
    }


    public AxisInsetsLayout(String group, String field, int axis, AxisInsets insets) {
        this(group, field, axis);
        this.insets = insets;
        // TODO Auto-generated constructor stub
    }
    
    public AxisInsetsLayout(String group, String field, int axis, Predicate filter, AxisInsets insets) {
        this(group, field, axis, filter);
        this.insets = insets;
        // TODO Auto-generated constructor stub
    }
     ...
     */
    public AxisInsetsLayout(String group, String field, int axis) {
        this(group, field);
        setAxis(axis);
        insets = new AxisInsets(0,0,0,0);
    }
     ...
     * to layout.
     */
    public AxisInsetsLayout(String group, String field, int axis, Predicate filter) {
        this(group, field, axis);
        setFilter(filter);
        insets = new AxisInsets(0,0,0,0);
    }
    ...
    public AxisInsets getInsets() {
        return insets;
    }

    public void setInsets(AxisInsets insets) {
        this.insets = insets;
    }
    ...
    protected void setMinMax() {
        Rectangle2D b = super.getLayoutBounds();
        if ( getAxis() == Constants.X_AXIS ) {
            m_min = b.getMinX() + insets.left;
            m_range = b.getMaxX() - m_min - insets.right;
        } else {
            m_min = b.getMaxY() - insets.bottom;
            m_range = b.getMinY() - m_min + insets.top;
        }
    }
    ...
    public Rectangle2D getLayoutBounds() {
        Rectangle2D b = super.getLayoutBounds();
        if ( getAxis() == Constants.X_AXIS ) {
            return new Rectangle2D.Double(b.getX() + insets.left, b.getY(), b.getWidth() - insets.left - insets.right, b.getHeight());
        } else {
            return new Rectangle2D.Double(b.getX(), b.getY() + insets.top, b.getWidth(), b.getHeight() - insets.top - insets.bottom);
        }
    }
    ...
    public double getRange() {
        ...
        case Constants.NUMERICAL:
        	if (m_model == null)
        		numericalLayout(ts);
        	return Math.abs(m_dist[0]-m_dist[1]);
        default:
        	if (m_model == null)
        		ordinalLayout(ts);
