...
/**
 * <p>TreeLayout instance that computes a radial space filling layout, 
...
public class StarburstLayout extends TreeLayout {
     ...
     * in the layout.
     */
    public StarburstLayout(String group, int radius) {
        this(group);
        m_radiusInc = radius;
        m_autoScale = false;
    }
    ...
    /**
     * Seth the width type, which defines how angular widths are divided among nodes.
     * @param widthType the new widthType to use for layout
     * @param widthField the float data field from which to take the counts for calculating the width -- childCount/parentCount = fraction of parent angle assigned to child
     */
    ...
    public void run(double frac) {
        ...
        // perform the layout
        if ( m_maxDepth > 0 )
            layout(n, rootRadiusHalf ? m_radiusInc/2 : m_radiusInc * 2, m_theta1, m_theta2);
        
        // update properties of the root node if we are laying out a Sector
        if (n.canGetDouble("angleExtent")) {
            n.setDouble("angleExtent", ((Params)n.get(PARAMS)).width);
            n.setDouble("startAngle", ((Params)n.get(PARAMS)).startAngle);
            n.setDouble("innerRadius", 0);
            // the root has half the radius of other levels
            n.setDouble("outerRadius", rootRadiusHalf ? m_radiusInc / 2 : m_radiusInc * 2);
        }
     ...
     */
    private double calcAngularWidth(NodeItem n, int d, double width, double angle) {
        if ( d > m_maxDepth ) m_maxDepth = d;       
        double aw = width;
        
        if (((Params)n.get(PARAMS)).size > 0) {
            synchronized(n) {
            ...
            while ( childIter.hasNext() ) {
                ...
                double childAW = aw * n.getDouble("angleFactor") * (((Params)c.get(PARAMS)).size / (((Params)n.get(PARAMS)).size));
        ...
        ((Params)n.get(PARAMS)).startAngle = angle;
        return aw;
    }
    ...
    protected void layout(NodeItem n, double r, double theta1, double theta2) {
        ...
        //double startAngle = ((Params)n.get(PARAMS)).startAngle;
        ...
        while ( childIter != null && childIter.hasNext() ) {
            ...
            if (c.canSetDouble("angleExtent")) {
                c.setDouble("angleExtent", cp.width);
                c.setDouble("startAngle", cp.startAngle);
                c.setDouble("innerRadius", r);
                c.setDouble("outerRadius", r+m_radiusInc);
            }
     ...
     */
    public static final String PARAMS = "_StarburstLayoutParams";
    /**
